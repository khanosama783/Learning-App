    ...
    try {
      ...
      if (isCacheExist) {
        ...
        const course = await CourseModel.findById(req.params.id).select(
          "-courseData.videUrl -course.Data.suggestion -courseData.questions -courseData.links"
        );
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await NotifactionModel.create({
        ...
        message: `You have a new quesion in ${courseContent.title}`,
      });
...
export const addAnswer = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { answer, courseId, contentId, quesionId }: IAddAnswerData =
        req.body;
